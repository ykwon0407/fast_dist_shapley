...
class DistShap(object):
    ...
    def __init__(self, X, y, X_test, y_test, num_test, X_tot=None, y_tot=None,
                 ...
                 overwrite=False, utility_minimum_samples=None,
                 sigma_2=None, **kwargs):
        """
        Args:
            X: Data covariates
        ...
        if self.is_regression or metric == 'l2':
            self.is_regression = True
            assert (sigma_2 is not None), 'Check sigma_2'
            assert (utility_minimum_samples is not None), 'Check utility_minimum_samples'
            self.sigma_2 = sigma_2
            self.utility_minimum_samples = utility_minimum_samples
            print("Regression problem is not implemented.")
    ...
    def _initialize_instance(self, X, y, X_test, y_test, num_test, 
        ...
        '''
        Remove data.pkl whenever this function is executed.
        '''
    ...
    def _load_dataset(self, data_dir):
        ...
        if 'X_tot' in data_dic.keys():
            ...
            self.X_tot = self.X
            self.y_tot = self.y
        (unique, counts) = np.unique(self.y_tot, return_counts=True)
        self.pi_hat = counts/np.sum(counts)
    ...
    def _find_experiment_number(self, directory):
        ...
        experiment_number = flag + experiment_number.zfill(5) # 6 -> 00006
    ...
    def value(self, model, metric=None, X=None, y=None):
        ...
        if metric == 'l2':
            ...
            return (2.*self.sigma_2 - np.mean((y - y_pred_tmp) ** 2))
        if metric == 'f1':
    ...
    def run(self, save_every, err, tolerance=None, truncation=None, alpha=None,
        ...
        while True:
            if dist_run:
                if error(self.results['mem_dist']) < err:
                    ...
                    self.vals_dist = np.mean(self.results['mem_dist'], 0) # TODO: Need to check
                    time_dist_run += time.time() - time_init
            if tmc_run:
                if error(self.results['mem_tmc']) < err:
                    ...
                    time_tmc_run += time.time() - time_init
            if self.directory is not None:
                print('save_results was removed')
                # self.save_results()
            if (tmc_run is not True) and (dist_run is not True):
                print('Reached to convergence!')
                break
        ...
        self.time_dist_run = time_dist_run
        self.time_tmc_run = time_tmc_run
        print('-'*30)
    ...
    def dist_iteration(self, truncation, sources=None, alpha=None):
        ...
        if self.is_regression and self.utility_minimum_samples is not None and k < self.utility_minimum_samples:
            return marginal_contribs, []
        S = np.random.choice(len(self.X_tot), k - 1)
        X_init = self.X_tot[S]
        y_init = self.y_tot[S]
        self.restart_model()
        if not self.is_regression and len(set(y_init)) != num_classes:
        ...
        for idx in range(len(sources.keys())):
            ...
            if not self.is_regression and len(set(y_batch)) != num_classes:
                continue
            self.restart_model()
            try:
    ...
    def tmc_iteration(self, tolerance, sources=None):
        ...
        for n, idx in enumerate(idxs):
            ...
            if self.is_regression and self.utility_minimum_samples is not None and n < self.utility_minimum_samples:
                continue
            if not self.is_regression and len(set(y_batch)) != num_classes:
                continue
            self.restart_model()
...
class DistShapDensity(object):
    ...
    def __init__(self, X, X_test, num_test, bandwidth, X_tot=None,
                 sources=None, problem='density',
                 sample_weight=None, directory=None, seed=None,
        ...
        if self.directory is not None:
            if not os.path.exists(directory):
                ...
                os.makedirs(os.path.join(directory, 'weights'))
                os.makedirs(os.path.join(directory, 'plots'))
            self._initialize_instance(X, X_test, num_test, X_tot, sources, sample_weight)
        self.model = KernelDensity(bandwidth=bandwidth, kernel='gaussian')
        self.random_score = self.init_score()
            
    def _initialize_instance(self, X, X_test, num_test, X_tot=None, sources=None, sample_weight=None):
        """Loads or creates sets of data."""
        data_dir = os.path.join(self.directory, 'data.pkl')
        '''
        Remove data.pkl whenever this function is executed.
        ...
        if os.path.exists(data_dir):
            os.remove(data_dir)
        self._save_dataset(data_dir, X, X_test, num_test, X_tot, sources, sample_weight)  
        self._load_dataset(data_dir)
        loo_dir = os.path.join(self.directory, 'loo.npy')
        self.vals_loo = None
        if os.path.exists(loo_dir):
    ...
    def _save_dataset(self, data_dir, X, X_test, num_test, X_tot, sources, sample_weight):
        '''Save the different sets of data if already does not exist.'''
        data_dic = {
            'X': X,
            'X_test': X_test[-num_test:], 
            'X_heldout': X_test[:-num_test],
        }
        if sources is not None:
            data_dic['sources'] = sources
        if X_tot is not None:
    ...
    def init_score(self):
        ...
        random_scores = []
        for _ in range(100):
            j = np.random.choice(len(self.X), 1)
            if self.sample_weight is None:
                self.model.fit(self.X[j].reshape(1,-1))
            else:
                self.model.fit(self.X[j].reshape(1,-1), sample_weight=self.sample_weight)
            random_scores.append(self.value(self.model))
        return np.mean(random_scores)
        
    def value(self, model, X=None):
        """Computes the values of the given model.
        Args:
            model: The model to be evaluated.
            metric: Valuation metric. If None the object's default
            ...
            """
        if X is None:
            random_ind = np.random.choice(len(self.X_test), 200)
            X = self.X_test[random_ind] # whole dataset requires too much time
        
        X_from_estimator = self.model.sample(len(X)) # sample from the density estimator
        risk = np.mean(np.exp(self.model.score_samples(X_from_estimator)))-2*np.mean(np.exp(self.model.score_samples(X))) # Utility computation
        utility = -risk
        return utility
    ...
    def _tol_mean_score(self):
        ...
        for _ in range(1):
            if self.sample_weight is None:
                ...
                self.model.fit(self.X, sample_weight=self.sample_weight)
            for _ in range(100):
                bag_idxs = np.random.choice(len(self.X_test), len(self.X_test))
                scores.append(self.value(
                    self.model, 
                    X=self.X_test[bag_idxs]
                ))
    ...
    def dist_iteration(self, truncation, sources=None, alpha=None):
        ...
        try:
            ...
            init_score = self.value(self.model)
        except:
            init_score = self.random_score
        ...
        for n, idx in enumerate(idxs):
            ...
            if True:
                if sample_weight_batch is None:
                    ...
                    self.model.fit(X_batch, sample_weight = sample_weight_batch)
                new_score = self.value(self.model)    
            #except:
                #continue
            marginal_contribs[sources[idx]] = new_score - old_score
            marginal_contribs[sources[idx]] /= len(sources[idx])
    ...
    def _calculate_loo_vals(self, sources=None, metric=None):
        ...
        for i in sources.keys():
            ...
            if self.sample_weight is None:
                ...
                self.model.fit(X_batch, sample_weight=sw_batch)
                
            removed_value = self.value(self.model)
            vals_loo[sources[i]] = (baseline_value - removed_value)
            vals_loo[sources[i]] /= len(sources[i])
        return vals_loo
