...
for dimension in [30]:
    for sample_size in [100]:
        for run_id in range(50):
            cmd = f"""#!/bin/bash
#
#SBATCH --job-name=[large_reg_time_{sample_size}_{dimension}_{run_id}]
...
python3 /oak/stanford/groups/mrivas/users/yckwon/repos/DistributionalShapley/main_time_regression.py --sample_size {sample_size} --run_id {run_id} --dimension {dimension} --is_DShapley
